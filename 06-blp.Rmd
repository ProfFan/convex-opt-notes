...
Let $\mathbb M_n$ be the probability distributions over labellings in $D^n$, i.e. $\mathbb M_n = \{ \mu \geq 0 | \sum_{x\in D^n} \mu(x) = 1\}$. We also denote $\Delta = \mathbb M_1$, which is the standard $(|D| - 1)$-dimensional simplex. The corners of $\Delta$ can be identified with elements in $D$. For a distribution $\mu \in \mathbb M_n$ and a variable $v\in \{1,\dots, n \}$ let $\mu_{[v]}\in\Delta$ be the marginal probability of distribution $\mu$ for $v$:
\begin{align}
...
for _ in range(10):
  A,B,C = rng.binomial(1,[0.67, 0.67, 1.0]).astype(np.int)
